...
def show_sidebar():
    with st.sidebar:
        ...
        # Show the server-enforced, persistent daily analysis allowance.
        quota_loaded, quota = SessionManager.get_analysis_quota()
        if quota_loaded:
            ...
            st.warning("Daily analysis limit is temporarily unavailable.")
        ...
        if persistent_session.is_configured():
            st.link_button(
                "Logout",
                persistent_session.public_login_url("logout"),
