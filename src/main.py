...
st.set_page_config(
    page_title="Health Insights Agent", page_icon="🩺", layout="wide"
)
...
def show_chat_history():
    ...
    if success:
        if upgrade_legacy_session_title(messages):
            st.rerun()
        for msg in messages:
...
def upgrade_legacy_session_title(messages):
    """Rename opened legacy sessions that still use the old date/time title."""
    current_session = st.session_state.get("current_session")
    if not current_session or not re.fullmatch(
    ...
    for message in messages:
        patient_match = re.fullmatch(
            r"Analyzing report for patient: (.+)", message.get("content", "")
        )
        if message.get("role") == "user" and patient_match:
            renamed, updated_session = st.session_state.auth_service.update_session_title(
                current_session["id"], build_report_title(patient_match.group(1))
            )
            if renamed:
                st.session_state.current_session = updated_session or {
                    **current_session,
                    "title": build_report_title(patient_match.group(1)),
                }
                return True
            return False
    return False
