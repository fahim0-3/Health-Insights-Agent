...
from services.medical_safety import add_medical_disclaimer
...
class AnalysisAgent:
    ...
    def check_rate_limit(self):
        """Check the database-backed daily limit without consuming a slot."""
        auth_service = st.session_state.get("auth_service")
        if not auth_service:
            return False, "Please sign in again."

        success, quota = auth_service.get_analysis_quota(ANALYSIS_DAILY_LIMIT)
        if not success:
            return False, quota
        if quota["remaining"] <= 0:
            return False, "Daily analysis limit reached. Please try again tomorrow."
        return True, None
    ...
    def analyze_report(self, data, system_prompt, check_only=False, chat_history=None):
        ...
        reserved, quota = auth_service.reserve_analysis_quota(ANALYSIS_DAILY_LIMIT)
        if not reserved:
            error_message = (
                quota
                if isinstance(quota, str)
                else "Daily analysis limit reached. Please try again tomorrow."
            )
        ...
        if result["success"]:
            ...
            result["content"] = add_medical_disclaimer(result["content"])
            self._update_analytics(result)
            self._update_knowledge_base(processed_data, result["content"])
        else:
            auth_service.release_analysis_quota()
